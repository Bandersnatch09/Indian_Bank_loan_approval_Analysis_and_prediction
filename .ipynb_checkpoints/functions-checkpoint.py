...
# Class instance of Loading the Dataset
class Loading:
    def __init__(self) :
        pass
...
#Class instance For Identifying and cleaning the Data     
class Cleaning:
    def __init__(self):
        pass
...
#class intance for creating plots
class Analysis:
    def __init__(self):
        pass
    ...
    # Analysis of Categorical columns by the risk flag
     for column in column_list:
        ...
        # Print column type and unique values for debugging
        print(f"Column Type: {dataset[column].dtype}")
        print(f"Unique Values in '{column}': {dataset[column].unique()}")
        
        # Check if Risk_Flag column exists
        if 'Risk_Flag' not in dataset.columns:
            print("Column 'Risk_Flag' not found in dataset.")
            continue
...
#Class instance for Creating classifier Mchine learning Models.
class Modeling:
    def __init__(self):
        pass
...
class Evaluation:
    ...
    def Evaluate(self,classifier, X_test, y_test):
        ...
        sns.heatmap(cm, annot=True, fmt='d', cmap='coolwarm', cbar=False,annot_kws={"size": 16, "weight": "bold", "color": "black"},
